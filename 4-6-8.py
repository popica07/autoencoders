...
def build_cae(input_shape=(32, 32, 3)): #input initial image 32x32 3 channels
    ...
    # encoder
    x = layers.Conv2D(4, kernel_size=3, padding="same", activation="relu")(inputs)   
    x = layers.MaxPooling2D(pool_size=2)(x)                                          

    x = layers.Conv2D(6, kernel_size=3, padding="same", activation="relu")(x)       
    x = layers.MaxPooling2D(pool_size=2)(x)                                          

    latent = layers.Conv2D(8, kernel_size=3, padding="same", activation="relu")(x)  
...
#latent space size
W, K, P, S, C = 8, 3, 1, 1, 8
print (conv2d_output_size(W, K, P, S, C)) #512 in this case