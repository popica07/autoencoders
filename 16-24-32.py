...
def build_cae(input_shape=(32, 32, 3)):
    ...
    x = layers.Conv2D(16, kernel_size=3, padding="valid", activation="relu")(inputs)   
    x = layers.MaxPooling2D(pool_size=2)(x)                                    

    x = layers.Conv2D(24, kernel_size=3, padding="same", activation="relu")(x)       
    x = layers.MaxPooling2D(pool_size=2)(x)                                          

    latent = layers.Conv2D(32, kernel_size=3, padding="same", activation="relu")(x)  
    ...
    #because of the valid padding I had to ad this
    x = layers.ZeroPadding2D(2)(x)# 32x32x12
...
#optional
# model checkpoint if validation loss improves
...
# Optional but helpful: stabilize training without cutting it short before 10 epochs
plateau = ReduceLROnPlateau(
    monitor="val_loss", factor=0.5, patience=3, min_lr=1e-5, verbose=1
)

#to stop training after no progress in 10 epochs
# Optional: keep but set patience >= 10 to respect “at least 10 epochs”
early = EarlyStopping(
...
#print loss on training and validation
print("\nTest Results")
print(f"loss: {results['loss']:.6f}")


#latent space size
W, K, P, S, C = 8, 3, 1, 1, 32

print (conv2d_output_size(W, K, P, S, C)) #2048